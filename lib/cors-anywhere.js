...
function proxyRequest(req, res, proxy) {
  ...
  console.log(`\nProxying request to host: ${location.host}`);
  console.log(`Full file: ${location.href}`);

  let hitCount = domainMap.get(location.host) || 0;
  hitCount++;

  domainMap.set(location.host, hitCount);

  if (hitCount > 1000 && hitCount % 1000 === 0) {
     console.log(`High hit count domain: ${location.href}, ${hitCount} hits`);
  }
